    ...
    size_t aRows, size_t aCols, size_t bCols)
{
    ...
    for (size_t k = 0; k < aCols; ++k) {
        for (size_t i = 0; i < aRows; ++i) {
            size_t j = 0;
            for (; j + 3 < bCols; j += 4) {
                out[bCols * i + j + 0] += a[aCols * i + k] * b[bCols * k + j + 0];
                out[bCols * i + j + 1] += a[aCols * i + k] * b[bCols * k + j + 1];
                out[bCols * i + j + 2] += a[aCols * i + k] * b[bCols * k + j + 2];
                out[bCols * i + j + 3] += a[aCols * i + k] * b[bCols * k + j + 3];
            }
            for (; j < bCols; ++j) {
    ...
    size_t aRows, size_t aCols)
{
  size_t i, j;
  for (i = 0; i < aRows; i++) {
    Elem tmp0 = 0, tmp1 = 0, tmp2 = 0, tmp3 = 0;
    size_t j = 0;
    for (; j + 3 < aCols; j += 4) {
      tmp0 += a[aCols*i + j + 0] * b[j + 0];
      tmp1 += a[aCols*i + j + 1] * b[j + 1];
      tmp2 += a[aCols*i + j + 2] * b[j + 2];
      tmp3 += a[aCols*i + j + 3] * b[j + 3];
    }
    for (; j < aCols; j++) {
      tmp0 += a[aCols*i + j] * b[j];
    }
    out[i] = tmp0 + tmp1 + tmp2 + tmp3;
  }
}
